...
def write_dashboards_yml() -> None:
    """Emit dashboards.yml: one provider per server (folderUid set by init-folders.sh)."""
...
def write_init_folders_sh() -> None:
    """Emit init-folders.sh to create Servers/<server> via Grafana API (parentUid)."""
    path = ROOT.parent / "init-folders.sh"
    lines = [
        "#!/bin/sh",
