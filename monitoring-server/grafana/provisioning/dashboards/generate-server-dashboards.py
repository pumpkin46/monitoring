...
def wrap_server_dashboards(server: str) -> None:
    """Wrap existing dashboards under servers/<server>/ for file provisioning."""
    ...
    for path in sorted(server_dir.glob("*.json")):
        with path.open(encoding="utf-8") as f:
            data = json.load(f)
        write_provisioned_dashboard(path, unwrap_dashboard(data))
...
def write_dashboards_yml() -> None:
    """Emit dashboards.yml for overview + per-server file provisioning."""
...
def main() -> None:
    ...
    for server, cfg in SERVERS.items():
        ...
        for name, dash in (("metrics", metrics), ("logs", logs)):
            ...
            write_provisioned_dashboard(path, dash)
