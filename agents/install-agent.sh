...
# Alloy runs as user 'alloy' — give it access to log files  
usermod -aG adm alloy 2>/dev/null || true   # adm group can read /var/log  
...
if command -v ufw &>/dev/null && ufw status | grep -q "active"; then  
  ufw allow out to $MONITORING_IP port 9090 proto tcp \  
    comment "Alloy → Prometheus"  
  ufw allow out to $MONITORING_IP port 3100 proto tcp \  
    comment "Alloy → Loki"  
...
if curl -s --connect-timeout 5 \  
  "http://${MONITORING_IP}:9090/-/healthy" | grep -q "Healthy"; then  
...
if curl -s --connect-timeout 5 \  
  "http://${MONITORING_IP}:3100/ready" | grep -q "ready"; then  
